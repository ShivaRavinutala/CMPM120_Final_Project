class Level3 extends Phaser.Scene {
    ...
    preload() {
        ...
        this.load.tilemapTiledJSON("dungeon3", "dungeon3.json");
        this.load.bitmapFont('font', 'Text.png', 'Text.xml');
    }

    create() {
        // player movement code
        this.map = this.add.tilemap("dungeon3",16,16, 25,25,); 
    ...
    update(time, delta) {
        ...
        enemies.forEach((enemy, index) => {
            ...
            if (Phaser.Geom.Intersects.RectangleToRectangle(enemy.getBounds(), this.player.getBounds())) {
                console.warn(`[Level3] Update: Collision detected between player and enemy at (${enemy.x.toFixed(2)}, ${enemy.y.toFixed(2)})!`);
                this.lives -= 1;
                console.log(`[Level3] Update: Player lives remaining: ${this.lives}`);
                enemy.destroy(); 
                console.log(`[Level3] Update: Enemy destroyed. Remaining enemies in group: ${this.enemyGroup.getLength()}`);
            }

            if (this.sword_attack_active) {
                if (Phaser.Geom.Intersects.RectangleToRectangle(enemy.getBounds(), this.sword.getBounds())) {
                    console.warn(`[Level3] Update: Collision detected between sword and enemy at (${enemy.x.toFixed(2)}, ${enemy.y.toFixed(2)})!`);
                    console.log(`[Level3] Update: Player lives remaining: ${this.lives}`);
            ...
            for (let x = 0; x < this.player_projectiles.length; x++) {
                if (Phaser.Geom.Intersects.RectangleToRectangle(enemy.getBounds(), this.player_projectiles[x].getBounds())) {
                    console.warn(`[Level3] Update: Collision detected between projectile and enemy at (${enemy.x.toFixed(2)}, ${enemy.y.toFixed(2)})!`);
                    console.log(`[Level3] Update: Player lives remaining: ${this.lives}`);
                    enemy.takeDamage();
                    console.log(`[Level3] Update: Enemy destroyed. Remaining enemies in group: ${this.enemyGroup.getLength()}`);
                }
    ...
    complete() {
        ...
        this.scene.start("BossLevel", {cur_level: 3, remaining_abilities: this.remaining_abilities, remaining_levels: this.remaining_levels});
    }
}
