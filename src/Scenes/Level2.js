        ...
        enemies.forEach((enemy, index) => {
            ...
            if (this.sword_attack_active) {
                if (Phaser.Geom.Intersects.RectangleToRectangle(enemy.getBounds(), this.sword.getBounds())) {
                    console.warn(`[Level2] Update: Collision detected between sword and enemy at (${enemy.x.toFixed(2)}, ${enemy.y.toFixed(2)})!`);
                    console.log(`[Level2] Update: Player lives remaining: ${this.lives}`);
            ...
            for (let x = 0; x < this.player_projectiles.length; x++) {
                if (Phaser.Geom.Intersects.RectangleToRectangle(enemy.getBounds(), this.player_projectiles[x].getBounds())) {
                    console.warn(`[Level2] Update: Collision detected between projectile and enemy at (${enemy.x.toFixed(2)}, ${enemy.y.toFixed(2)})!`);
                    console.log(`[Level2] Update: Player lives remaining: ${this.lives}`);
                    enemy.takeDamage();
                    console.log(`[Level2] Update: Enemy destroyed. Remaining enemies in group: ${this.enemyGroup.getLength()}`);
                }
